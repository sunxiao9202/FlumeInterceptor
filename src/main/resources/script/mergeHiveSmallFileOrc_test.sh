...
set mapreduce.job.name=ods.cc_studied_lesson_log_orc;
create table if not exists ods.cc_studied_lesson_log_orc(
Id int COMMENT '自增ID',
...
insert overwrite table ods.cc_studied_lesson_log_orc partition(PARTITION_TIME)
select Id,USER_ID,RECRUIT_ID,LESSON_ID,LESSON_VIDEO_ID,VIDEO_ID,PLAY_TIMES,STUDY_TOTAL_TIME,LEARN_TIME,SOURSE_TYPE,CREATE_TIME,PARTITION_TIME from ods.cc_studied_lesson_log;
